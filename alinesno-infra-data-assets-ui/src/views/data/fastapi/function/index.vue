   ...
   <div class="app-container">
      <el-row :gutter="20">
         ...
         <el-col :span="24" :xs="24">
            ...
            <el-table v-loading="loading" :data="ApiFunctionList" @selection-change="handleSelectionChange">
               ...
               <el-table-column label="操作" align="center" width="150" class-name="small-padding fixed-width">
                  <template #default="scope">
                     <el-tooltip content="修改" placement="top" v-if="scope.row.ApiFunctionId !== 1">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                           v-hasPermi="['system:ApiFunction:edit']"></el-button>
                     </el-tooltip>
                     <el-tooltip content="删除" placement="top" v-if="scope.row.ApiFunctionId !== 1">
                        <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                           v-hasPermi="['system:ApiFunction:remove']"></el-button>
                     </el-tooltip>
                  </template>
...
import {
   ...
   updateApiFunction,
   addApiFunction
} from "@/api/data/fastapi/apiFunction";

const router = useRouter();
const { proxy } = getCurrentInstance();

// 定义变量
const ApiFunctionList = ref([]);
...
function getList() {
   loading.value = true;
   listApiFunction(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
      loading.value = false;
      ApiFunctionList.value = res.rows;
...
function handleDelete(row) {
   const ApiFunctionIds = row.id || ids.value;
   proxy.$modal.confirm('是否确认删除应用编号为"' + ApiFunctionIds + '"的数据项？').then(function () {
      return delApiFunction(ApiFunctionIds);
   }).then(() => {
      getList();
...
function handleUpdate(row) {
   reset();
   const ApiFunctionId = row.id || ids.value;
   getApiFunction(ApiFunctionId).then(response => {
...
function submitForm() {
   proxy.$refs["databaseRef"].validate(valid => {
      if (valid) {
         if (form.value.ApiFunctionId != undefined) {
            updateApiFunction(form.value).then(response => {
               proxy.$modal.msgSuccess("修改成功");
               open.value = false;
               getList();
            });
         } else {
            addApiFunction(form.value).then(response => {
