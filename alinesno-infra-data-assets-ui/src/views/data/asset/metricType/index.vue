...
<script setup name="MetricType">
import { listMetricType, getMetricType, delMetricType, addMetricType, updateMetricType, listMetricTypeExcludeChild } from "@/api/data/asset/metricType";
...
function getList() {
  loading.value = true;
  listMetricType(queryParams.value).then(response => {
...
function handleUpdate(row) {
  reset();
  listMetricTypeExcludeChild(row.id).then(response => {
    deptOptions.value = proxy.handleTree(response.data, "id");
  });
  getMetricType(row.id).then(response => {
  ...
  proxy.$refs["deptRef"].validate(valid => {
    if (valid) {
      if (form.value.id != undefined) {
        updateMetricType(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addMetricType(form.value).then(response => {
