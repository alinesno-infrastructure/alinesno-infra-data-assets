...
function handleUpdate(row) {
  reset();
  listMetricExcludeChild(row.id).then(response => {
    deptOptions.value = proxy.handleTree(response.data, "id");
  });
  getMetric(row.id).then(response => {
  ...
  proxy.$refs["deptRef"].validate(valid => {
    if (valid) {
      if (form.value.id != undefined) {
        updateMetric(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addMetric(form.value).then(response => {
